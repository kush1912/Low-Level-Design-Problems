...
public class L04_Synchronized {
    ...
    //
    // IMPORTANT: Separate locks are SAFE only when resources are INDEPENDENT.
    //   Independent (count1, count2):    separate locks OK (parallel, fast)
    //   Related (account1, account2):    MUST use same lock (consistent, slower)
    //
    //   BAD — separate locks for related data:
    //     synchronized (lock1) { account1 -= amount; }  // Step 1
    //     // GAP: another thread sees money "disappeared" here!
    //     synchronized (lock2) { account2 += amount; }  // Step 2
    //
    //   FIX — one lock for related data:
    //     synchronized (transferLock) {
    //         account1 -= amount;
    //         account2 += amount;  // both protected atomically
    //     }
    ...
    //   Java's intrinsic lock tracks WHICH thread holds it AND a hold count:
    //
    //   methodA() called by Thread-1:
    //     Lock holder: Thread-1, holdCount: 1       <- acquired
    //     methodB() called inside methodA:
    //       Lock holder: Thread-1, holdCount: 2     <- same thread, just increment
    //       methodB finishes -> holdCount: 1        <- decrement
    //     methodA finishes -> holdCount: 0           <- fully released
    //
    // IMPORTANT: synchronized IS reentrant by itself!
    //   You do NOT need ReentrantLock for reentrancy.
    //   Both synchronized and ReentrantLock are reentrant.
    //   ReentrantLock is named "Reentrant" to emphasize it PRESERVES this
    //   behavior while adding extra features (tryLock, timeout, fairness, Conditions)
    //   — not because synchronized lacks it.
